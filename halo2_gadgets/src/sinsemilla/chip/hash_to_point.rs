    ...
    Commit: CommitDomains<pallas::Affine, Fixed, Hash>,
{
    ...
    > {
        ...
        let mut zs_sum: Vec<Vec<AssignedCell<Assigned<pallas::Base>, pallas::Base>>> = Vec::new();
                ...
                .assert_if_known(|(field_elems, (x_a, y_a))| {
                    ...
                    let bitstring: Vec<bool> = field_elems
                        ...
                        .flat_map(|(elem, num_words)| {
                            elem.evaluate().to_le_bits().into_iter().take(K * num_words)
                        })
                        .collect();
    ...
    fn hash_piece(
        ...
        (
            X<pallas::Base>,
            Y<pallas::Base>,
            Vec<AssignedCell<Assigned<pallas::Base>, pallas::Base>>,
        ),
        Error,
    > {
        let config = self.config().clone();
        ...
        let zs = {
            let mut zs: Vec<AssignedCell<Assigned<pallas::Base>, pallas::Base>> =
                Vec::with_capacity(piece.num_words() + 1);
